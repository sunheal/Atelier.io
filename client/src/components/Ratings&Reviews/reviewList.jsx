...
class ReviewList extends React.Component {
  ...
  getReviewinfo() {
    axios
      .get(`/reviews/?product_id=${this.props.id}&count=5000`)
      .then((output) => {
  ...
  render() {
    return (
      <div>
        {this.state.currentReview.length === 0 ? (
          <div>
          ...
          <div>
            <p className="reviewsCount">
              {" "}
              {this.state.currentReview.length} Reviews, sorted by
            ...
            {this.state.currentReview.length !==
            this.state.whatShowing.length ? (
              this.state.currentReview.length > 2 ? (
                <button className="moreReview" onClick={this.appendReview}>
                  {" "}
                  More Review{" "}
                </button>
              ) : null
            ) : null}
            <button className="addReview" onClick={this.onShowModal}>
              {" "}
              Add Review{" "}
            </button>
